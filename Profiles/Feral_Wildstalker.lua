...
local Profile = {
    ...
    rules = {
        ...
        -- Assisted Combat sometimes keeps surfacing Cat Form as a utility
        -- suggestion even while the player is already shapeshifted into Cat.
        -- Suppress that duplicate form button locally.
        {
        ...
        -- Mark of the Wild should not keep resurfacing while its player buff
        -- is already active.
        {
            type = "BLACKLIST_CONDITIONAL",
        ...
        -- Rake is a target DoT, not a button to spam repeatedly into the same
        -- target. Suppress it on the current target until the local pandemic
        -- refresh window, using a cast-tracked per-target timer.
        {
            type = "BLACKLIST_CONDITIONAL",
            spellID = SPELLS.Rake,
